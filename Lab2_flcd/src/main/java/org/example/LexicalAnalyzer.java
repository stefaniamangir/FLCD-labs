...
public class LexicalAnalyzer {
    ...
    private FiniteAutomataAnalyzer constantFiniteAutomata = new FiniteAutomataAnalyzer("FA_constants");
    private FiniteAutomataAnalyzer identifierFiniteAutomata = new FiniteAutomataAnalyzer("FA_identifiers");
    ...
    public LexicalAnalyzer(SymbolTable identifiersSymbolTable, SymbolTable constantsSymbolTable) throws IOException {
        this.identifiersSymbolTable = identifiersSymbolTable;
        this.constantsSymbolTable = constantsSymbolTable;
        this.lexical_error = false;
        //this.constantFiniteAutomata.run("FA_constants");
        //this.identifierFiniteAutomata.run("FA_identifiers");
    }
    ...
    private boolean processToken(String token) {
        ...
        if (RESERVED_WORDS.contains(token) || OPERATORS.contains(token) || SEPARATORS.contains(token)) {
            addToPIF(token, -1);
            return true;
        } else if (identifierFiniteAutomata.isAccepted(token)) {
            int index = identifiersSymbolTable.addElement(token);
            addToPIF("id", index);
            return true;
        } else if (constantFiniteAutomata.isAccepted(token)) {
